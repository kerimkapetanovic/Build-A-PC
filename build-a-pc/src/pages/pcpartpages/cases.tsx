...
function CaseSelection() 
{
  ...
  const handleComponentSelect=(caseItem) => 
  {
        ...
        <div className="tw-flex tw-flex-col tw-items-center fade-in">
          ...
          <div className="tw-grid tw-grid-cols-4 tw-gap-4">
            {caseData.map((caseItem) => (
              <div key={caseItem.name} className="tw-flex tw-flex-col tw-items-center tw-bg-white tw-p-4 tw-rounded tw-shadow-md">
                ...
                <div className="tw-text-black tw-text-lg">${caseItem.price}</div>
                <button className={`tw-bg-blue-500 tw-text-white tw-px-4 tw-py-2 tw-rounded tw-text-lg tw-mt-4 ${selectedCase === caseItem ? 'tw-border-4 tw-border-blue-500' : ''}`} onClick={() => handleComponentSelect(caseItem)}>
                  Add to Cart
                </button>
              </div>
            ))}
          ...
          {selectedCase && (
            <div className="tw-mt-8 tw-text-white">
              <h3 className="tw-text-2xl">Selected Case:</h3>
              <p className="tw-text-xl">{selectedCase.name}</p>
              <p className="tw-text-xl">${selectedCase.price}</p>
            </div>
          )}
        </div>
      </div>
